...
|---|---|---|
| AZ-001 | Fixed; live-gated | Caddy's admin API binds only container loopback, is never published in bridge mode, and is reached exclusively by exact `podman exec` curl calls. Boot migrates a running legacy proxy only after a recoverable secure snapshot. Covered by command-shape, runtime-drift, first-deploy, and legacy-recreation integration tests. |
...
| AZ-015 | Fixed; live-gated | Per-role Quadlets preserve command/env/options/labels and use systemd-safe paths and encoding. Caddy starts non-root from an Azud-owned mode-0600 recovery file in its config volume, without mounting protected host state. Imperative and Quadlet paths share the same read-only, least-capability, bounded-resource runtime. The no-start transition keeps the proxy online while handing sole reboot authority from Podman's restart service to Quadlet. Unit generation, supervisor-policy, and cold-start integration cover the path. |
