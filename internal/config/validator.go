...
func Validate(cfg *Config) error {
	...
	if len(cfg.Servers) == 0 {
		...
		for role, rc := range cfg.Servers {
			...
			default:
				errs = append(errs, ValidationError{
					Field:   fmt.Sprintf("servers.%s.strategy", role),
					Message: fmt.Sprintf("strategy must be %s or %s", StrategyRolling, StrategyStopFirst),
				})
			}

			runtimeField := fmt.Sprintf("servers.%s.runtime", role)
			if rc.Runtime.DisableHealthcheck && role == WebRoleName {
				// The web role's HEALTHCHECK is generated by Azud from the
				// liveness probe and is the signal behind deployment gating
				// and health reporting, so it must not be suppressed.
				errs = append(errs, ValidationError{
					Field:   runtimeField + ".disable_healthcheck",
					Message: "disable_healthcheck is supported only for non-web roles",
				})
			}
	...
	if cfg.HasRole(WebRoleName) && cfg.Proxy.Host == "" && len(cfg.Proxy.Hosts) == 0 {
		errs = append(errs, ValidationError{
			Field:   "proxy.host",
	...
	if cfg.HasRole(WebRoleName) && cfg.Podman.Rootless && !cfg.Proxy.Rootful {
	...
	if cfg.HasRole(WebRoleName) && cfg.UseHostPortUpstreams() {
